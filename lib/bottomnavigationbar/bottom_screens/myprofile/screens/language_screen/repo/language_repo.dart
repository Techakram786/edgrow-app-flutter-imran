...
class LanguageRepository{
  Future<dynamic> getLanguage(String token) async {
    ...
    try {
      API api = API(url: ApiUrl.baseUrl + ApiUrl.endpointOnGetLanguage);
      api.get(
        bearerToken: token,
  ...
  Future<dynamic> updateLanguage({required String token,required var params}) async {

    Completer<dynamic> completer = Completer<dynamic>();

    try {
      API api = API(url: ApiUrl.baseUrl + ApiUrl.endpointOnUpdateLanguage);
      api.post(
        bearerToken: token,
        rawData:params,
        onApiSuccess: (response) {
