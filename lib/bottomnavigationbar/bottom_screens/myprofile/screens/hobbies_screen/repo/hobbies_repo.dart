...
class HobbiesRepository{
  ...
  Future<dynamic> getHobbies(String token) async {
    ...
    try {
      API api = API(url: ApiUrl.baseUrl + ApiUrl.endpointOnGetHobbies);
      api.get(
        bearerToken: token,
  ...
  Future<dynamic> updateHobbies({required String token,required var params}) async {

    Completer<dynamic> completer = Completer<dynamic>();

    try {
      API api = API(url: ApiUrl.baseUrl + ApiUrl.endpointOnUpdateHobbies);
      api.post(
        bearerToken: token,
        rawData:params,
        onApiSuccess: (response) {
